...
def generate_index():
    ...
    for _, wf in validated_workflows:
        ...
        rows.append(f"| [{name}](./docs/_workflows/{name}.md) | {domain} | {maintainer} | {systems} | {status_str} |\n")

    body = header + "".join(rows) + footer
    ...
    with open("docs/workflows.md", "w", encoding="utf-8") as f:
        f.write(body.replace("](./docs/_workflows/", "](./_workflows/"))
